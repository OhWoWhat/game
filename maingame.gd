...
@onready var volcano_camera = $CutsceneCamera/Camera3D2
@onready var volcano_animation: AnimationPlayer = $CutsceneCamera/Camera3D2/VolcanoAnimation
...
@onready var volcano_audio: AudioStreamPlayer = $Sounds/VolcanoAudio
@onready var volcano_smoke: GPUParticles3D = $Nature/PushilinVolcano/VolcanoSmoke
@onready var rain_particles: GPUParticles3D = $RainParticles
@onready var minimap_ui: CanvasLayer = $Map
@onready var ashfall: GPUParticles3D = $Ashfall
...
@export var current_map_name: String = "BrgyTibay"
...
var molten_debris_scene := preload("res://DisastersAssets/Final/MoltenDebris.tscn")
...
func _ready():
	...
	# ✅ Connect volcano animation finish signal (for returning camera control)
	if volcano_animation:
		volcano_animation.animation_finished.connect(_on_volcano_animation_finished)
...
var molten_debris_spawn_min = 3.0
var molten_debris_spawn_max = 6.0
...
func apply_difficulty_settings():
	match difficulty_level:
		"Easy":
			...
			molten_debris_spawn_min = 4.0
			molten_debris_spawn_max = 7.0
		...
		"Normal":
			...
			molten_debris_spawn_min = 3.0
			molten_debris_spawn_max = 6.0
		...
		"Hard":
			...
			molten_debris_spawn_min = 1.0
			molten_debris_spawn_max = 2.0
...
func start_disaster_sequence():
	...
	if current_disaster == "":
		var disasters = ["earthquake","typhoon","volcano","tsunami"]
	...
	match current_disaster:
		...
		"earthquake":
			...
			start_earthquake()
		"volcano":
			await show_alert("🌋 Volcanic eruption incoming! Look for face mask and Watch out for falling magma rocks!")
			start_volcano()
		"tsunami":
...
func start_typhoon():
	...
	apply_permanent_distortion(1.2)
...
func start_debris_gusts():
	if debris_gust_timer and debris_gust_timer.is_inside_tree():
		debris_gust_timer.queue_free()

	debris_gust_timer = Timer.new()
...
func start_earthquake():
	...
	switch_to_disaster_bgm()
	alarm_sound.play()
	apply_permanent_distortion(2.0)
	
	if earthquake_audio:
		earthquake_audio.volume_db = -80
		earthquake_audio.play()
		var tween = create_tween()
		tween.tween_property(earthquake_audio, "volume_db", 0, 20.0)

	if player.has_method("start_earthquake"):
		player.start_earthquake(60, .2)
	begin_survival_timer()
	spawn_loot()
	make_trees_fall()
...
func spawn_falling_debris():
	...
	var offset = Vector3(randf_range(-10, 5), 20, randf_range(-10, 5))
	debris_instance.global_transform.origin = player.global_transform.origin + offset
...
func apply_permanent_distortion(strength: float = 1.5):
	if not player_camera:
		return

	distortion_active = true
	target_fov = normal_fov + (10 * strength)
	...
	distortion_tween.tween_property(player_camera, "fov", target_fov, 1.5)\
		.set_trans(Tween.TRANS_SINE)\
...
func remove_distortion():
	...
	var tween = create_tween()
	tween.tween_property(player_camera, "fov", normal_fov, 1.5)\
		.set_trans(Tween.TRANS_SINE)\
...
func start_volcano():
	print("Volcano eruption started!")
	emit_signal("disaster_started", "volcano")
	switch_to_disaster_bgm()
	apply_permanent_distortion(1.5)

	minimap_ui.visible = false
	# 🎥 Switch to volcano camera
	if volcano_camera:
		volcano_camera.current = true
		
	# 🎬 Play volcano animation
	if volcano_animation and not volcano_animation.is_playing():
		volcano_animation.play("VolcanoCutscene")

	if world_environment and world_environment.environment:
		await transition_sky(eruption_sky)
	# 🌋 Play volcano audio
	if volcano_audio:
		volcano_audio.volume_db = -80
		volcano_audio.play()
		var tween = create_tween()
		tween.tween_property(volcano_audio, "volume_db", 0, 6.0)
	...
	if ashfall:
		...
		ashfall.amount = 1
		var particle_tween = create_tween()
		particle_tween.tween_property(ashfall, "amount", 100000, 3)
		# Start ashfall damage
		if player.has_node("AshfallDamageTimer"):
			var ash_timer = player.get_node("AshfallDamageTimer")
			ash_timer.start()

	begin_survival_timer()
	spawn_loot()
	var molten_timer := Timer.new()
	molten_timer.name = "MoltenDebrisTimer"
	molten_timer.wait_time = randf_range(molten_debris_spawn_min, molten_debris_spawn_max)
	molten_timer.one_shot = false
	molten_timer.timeout.connect(_on_molten_timer_timeout)
	add_child(molten_timer)
	molten_timer.start()
func _on_molten_timer_timeout():
	spawn_molten_debris()
	var timer = get_node_or_null("MoltenDebrisTimer")
	if timer:
		timer.wait_time = randf_range(molten_debris_spawn_min, molten_debris_spawn_max)
		timer.start()

func spawn_molten_debris():
	var molten = molten_debris_scene.instantiate()
	add_child(molten)
	var offset = Vector3(randf_range(-20, 20), 30, randf_range(-20, 20))
	molten.global_transform.origin = player.global_transform.origin + offset
...
func start_tsunami():
	...
	print("Tsunami wave finished. Waiting 3 seconds...")
...
# 🔁 When volcano animation finishes, return control to player
func _on_volcano_animation_finished(anim_name: StringName):
	if anim_name == "VolcanoCutscene":
		player_camera.current = true
		minimap_ui.visible = true
		alarm_sound.play()
...
func _on_retry_pressed() -> void:
	...
	player.global_transform.origin = Vector3(-201.845, 0.33, 116.3)

	# Reset sounds and particles
	...
	# Clean up timers and debris
	survival_timer.stop()

	for node in get_tree().get_nodes_in_group("falling_debris"):
		node.queue_free()

	for debris in get_tree().get_nodes_in_group("molten_debris"):
		debris.queue_free()

	var molten_timer = get_node_or_null("MoltenDebrisTimer")
	if molten_timer:
		molten_timer.stop()
		molten_timer.queue_free()

	var debris_timer = get_node_or_null("FallingDebrisTimer")

	# Restart the same disaster
	match current_disaster:
		"tsunami":
			var sea_rising_anim: AnimationPlayer = $Nature/Sea/SeaRising
			if sea_rising_anim:
